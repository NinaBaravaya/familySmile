...
if (isset($_POST['Delete']))
{
    ...
    if (!empty($id)){
        ...
        $sql_insert = "DELETE FROM `price` WHERE `id`='$id'";// $id не из базы, а то что содержиться в переменной post в поле, которое мы редактируем
    ...
    <table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1150" height="600" cellpadding="0" cellspacing="0">
        <tr valign="top">
            ...
            <td style="padding-left: 30px;">
                ...
                <form action="edit_price.php" method="post" enctype="multipart/form-data">
                    <table border="1" bordercolor="#e2e2e2" width="1100" height="100" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            <td width="50px">id</td><td width="250px">Path</td><td>Alt</td><td></td>
                        </tr>
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            <td><input name="id" type="text" value="<?php echo $str['id']; ?>" size="1"></td>
                            <td><input name="Path" type="text" value="<?php echo $str['Name']; ?>" size="25"></td>
                            <td><input name="Alt" type="text" value="<?php echo $str['Price']; ?>" size="25"></td>
