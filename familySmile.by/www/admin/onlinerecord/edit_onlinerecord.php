...
if (isset($_POST['Save']))
{
    ...
    if (!empty($id)){
        ...
        $sql_insert = "UPDATE `onlinerecord` SET `Name`='$Name',`Surname`='$Surname',`Phone`='$Phone',`Email`='$Email',`Date`='$Date',`Message`='$Message',`Status`='$Status' WHERE `id`='$id'";// $id не из базы, а то что содержиться в переменной post в поле, которое мы редактируем
        $date_insert = mysql_query($sql_insert);

        if ($date_insert == 'true')
        {
            header('Location:view_onlinerecord.php');
...
<table border="1" bordercolor="#e2e2e2" width="180" height="600" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">
    ...
    <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
        <td><form action="view_onlinerecord.php" method="POST" enctype="text/plain"><input name="Slider" type="submit" value="Запись-онлайн"></form></td>
    </tr>
    <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
    ...
    <table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1600" height="600" cellpadding="0" cellspacing="0">
        <tr valign="top">
            ...
            <td style="padding-left: 30px;">
                <h2><center><b> Запись-онлайн </b></center></h2>
                <?php
                $itemId = $_POST['edit'];//это и есть постбэк? т.е. достаем значение кнопки у edit которую мы нажали на view_slider.php
                $SQL = "SELECT `id`, `Name`, `Surname`, `Phone`, `Email`, `Date`, `Message`, `Status` FROM `onlinerecord` WHERE `id`=".$itemId;//то что содержиться в value у кнопки редактировать на странице view_slider.php
                ...
                <form action="edit_onlinerecord.php" method="post" enctype="multipart/form-data">
                    <table border="1" bordercolor="#e2e2e2" width="1600" height="100" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            <td width="50px">id</td><td width="120px">Name</td><td>Surname</td><td>Phone</td><td>Email</td><td>Date</td><td width="300px">Message</td><td>Status</td><td></td>
                        </tr>
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            <td><input name="id" type="text" value="<?php echo $str['id']; ?>" size="1"></td>
                            <td><input name="Name" type="text" value="<?php echo $str['Name']; ?>" size="10"></td>
                            <td><input name="Surname" type="text" value="<?php echo $str['Surname']; ?>" size="15"></td>
                            <td><input name="Phone" type="text" value="<?php echo $str['Phone']; ?>" size="15"></td>
                            <td><input name="Email" type="text" value="<?php echo $str['Email']; ?>" size="15"></td>
                            <td><input name="Date" type="text" value="<?php echo $str['Date']; ?>" size="15"></td>
                            <td><input name="Message" type="text" value="<?php echo $str['Message']; ?>" size="25"></td>
                            <td><input name="Status" type="text" value="<?php echo $str['Status']; ?>" size="10"></td>
