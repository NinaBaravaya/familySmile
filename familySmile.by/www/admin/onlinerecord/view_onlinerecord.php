...
<table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1600" height="600" cellpadding="0" cellspacing="0">
    ...
    <tr valign="top">
        <td style="padding-left: 30px;">
            ...
            <?php
            $SQL = "SELECT `id`, `Name`, `Surname`, `Phone`, `Email`, `Date`, `Message`, `Status` FROM `onlinerecord`";
            $data = mysql_query($SQL);
            $count = mysql_affected_rows();
            $inf .='<table border="1" bordercolor="#e2e2e2" width="1600" height="150px" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">';
            $inf .='<tr style="text-align: center; background-color: #9bd7d5; color: #fff;">
                    <td width="50px">id</td><td width="100px">Name</td><td>Surname</td><td>Phone</td><td>Email</td><td>Date</td><td width="300px">Message</td><td>Status</td><td></td></tr>';

            for($i=0;$i<$count;$i++)
            {
                $str=mysql_fetch_array($data);

                $inf .='<tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 18px;">';
                $inf .='<td width="50px">' .$str['id'].'</td>';
                $inf .='<td width="100px">' .$str['Name'].'</td>';
                $inf .='<td width="150px">' .$str['Surname'].'</td>';
                $inf .='<td width="250px">' .$str['Phone'].'</td>';
                $inf .='<td width="150px">' .$str['Email'].'</td>';
                $inf .='<td width="250px">' .$str['Date'].'</td>';
                $inf .='<td width="300px">' .$str['Message'].'</td>';
                $inf .='<td width="150px">' .$str['Status'].'</td>';

                $inf .= '<td><form action="edit_onlinerecord.php" method="POST"><button type="submit" name="edit" value="'.$str['id'].'">Редактировать</button></form></td>';
