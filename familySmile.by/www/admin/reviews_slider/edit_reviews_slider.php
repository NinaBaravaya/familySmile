...
if (isset($_POST['Save']))
{
    ...
    if (!empty($id)){
        ...
        $sql_insert = "UPDATE `reviews_slider` SET `Name`='$Name',`Surname`='$Surname',`Phone`='$Phone',`One_word`='$One_word' WHERE `id`='$id'";// $id не из базы, а то что содержиться в переменной post в поле, которое мы редактируем
        $date_insert = mysql_query($sql_insert);

        if ($date_insert == 'true')
        {
            header('Location:view_reviews_slider.php');
    ...
    <table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1150" height="600" cellpadding="0" cellspacing="0">
        <tr valign="top">
            ...
            <td style="padding-left: 30px;">
                <h2><center><b> Отзывы пациентов </b></center></h2>
                <?php
                $itemId = $_POST['edit'];//это и есть постбэк? т.е. достаем значение кнопки у edit которую мы нажали на view_slider.php
                $SQL = "SELECT `id`, `Name`, `Surname`, `Phone`, `One_word` FROM `reviews_slider` WHERE `id`=".$itemId;//то что содержиться в value у кнопки редактировать на странице view_slider.php
                ...
                <form action="edit_slider.php" method="post" enctype="multipart/form-data">
                    <table border="1" bordercolor="#e2e2e2" width="1100" height="100" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            <td width="50px">id</td><td width="150px">Name</td><td>Surname</td><td>Phone</td><td>One_word</td><td></td>
                        </tr>
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            ...
                            <td><input name="One_word" type="text" value="<?php echo $str['One_word']; ?>" size="25"></td>
