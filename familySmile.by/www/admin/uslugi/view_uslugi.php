...
<table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1150" height="600" cellpadding="0" cellspacing="0">
    ...
    <tr valign="top">
        <td style="padding-left: 30px;">
            ...
            <?php
            $SQL = "SELECT `id`, `Name`, `Path`, `Alt`, `Title` FROM `uslugi`";
            ...
            $inf .='<tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                    <td width="50px">id</td><td width="200px">Name</td><td width="250px">Path</td><td width="150px">Alt</td>
                    <td width="150px">Title</td><td></td></tr>';
            ...
            for($i=0;$i<$count;$i++)
            {
                ...
                $inf .='<td width="200px">' .$str['Name'].'</td>';
                $inf .='<td width="250px">' .$str['Path'].'</td>';
                $inf .='<td width="150px">' .$str['Alt'].'</td>';
                $inf .='<td width="150px">' .$str['Title'].'</td>';

                $inf .= '<td><form action="edit_uslugi.php" method="POST"><button type="submit" name="edit" value="' .$str['id'].'">Редактировать</button></form></td>';
