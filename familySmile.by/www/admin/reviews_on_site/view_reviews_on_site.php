...
<table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1470" height="600" cellpadding="0" cellspacing="0">
    ...
    <tr valign="top">
        <td style="padding-left: 30px;">
            ...
            <?php
            $SQL = "SELECT `id`, `Name`,`Surname`, `Date`,`What_like`, `Path` FROM `reviews_form`";


            $data = mysql_query($SQL);
            $count = mysql_affected_rows();
            $inf .='<table border="1" bordercolor="#e2e2e2" width="1470" height="200px" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">';
            $inf .='<tr style="text-align: center; background-color: #9bd7d5; color: #fff;">
                    <td width="10px">id</td><td width="50px">Name</td><td width="50px">Surname</td><td width="40px">Date</td>
                    <td width="250px">What_like</td><td width="100px">Path</td><td width="50px"></td><td width="50px"></td></tr>';
            ...
            for($i=0;$i<$count;$i++)
            {
                ...
                $inf .= '<td><form action="edit_reviews_on_site.php" method="POST"><button type="submit" name="edit" value="' .$str['id'].'">Редактировать</button></form></td>
                <td><form action="delete_reviews_on_site.php" method="POST"><button name="delete" type="submit" value="'.$str['id'].'">Удалить</button></form></td>';
            }
            $inf .='</tr></table>';
            echo $inf;
            ?>
