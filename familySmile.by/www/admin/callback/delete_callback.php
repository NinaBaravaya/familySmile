...
if (isset($_POST['Delete']))
{
    ...
    if (!empty($id)){
        ...
        $sql_insert = "DELETE FROM `callback` WHERE `id`='$id'";// $id не из базы, а то что содержиться в переменной post в поле, которое мы редактируем
        $date_insert = mysql_query($sql_insert);

        if ($date_insert == 'true')
        {
            header('Location:view_callback.php');
...
<table border="1" bordercolor="#e2e2e2" width="180" height="600" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">
    ...
    <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
        <td><form action="view_callback.php" method="POST" enctype="text/plain"><input name="callback" type="submit" value="Заказать звонок"></form></td>
    </tr>
    <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
    ...
    <table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1150" height="600" cellpadding="0" cellspacing="0">
        <tr valign="top">
            ...
            <td style="padding-left: 30px;">
                <h2><center><b> Заказать звонок </b></center></h2>
                <?php
                $itemId = $_POST['delete'];//это и есть постбэк, т.е. достаем значение кнопки у edit которую мы нажали на view_slider.php
                $SQL = "SELECT `id`, `Name`, `Phone`, `Status` FROM `callback` WHERE `id`=".$itemId;//то что содержиться в value у кнопки редактировать на странице view_slider.php
                ...
                <form action="delete_callback.php" method="post" enctype="multipart/form-data">
                    <table border="1" bordercolor="#e2e2e2" width="1100" height="100" cellpadding="0" cellspacing="0" style=" table-layout:fixed; float:left; margin:0 9px 0 0;">
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            <td width="50px">id</td><td width="250px">Name</td><td>Phone</td><td>Status</td><td></td>
                        </tr>
                        <tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                            ...
                            <td><input name="Phone" type="text" value="<?php echo $str['Phone']; ?>" size="25"></td>
                            <td><input name="Status" type="text" value="<?php echo $str['Status']; ?>" size="25"></td>
                            <td><input name="Delete" type="submit" value="удалить запись"></td>
                        </tr>
                    </table>
                </form>
