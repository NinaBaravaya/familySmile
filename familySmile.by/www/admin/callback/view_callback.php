...
<table border="1" bgcolor="#e2e2e2" bordercolor="#e2e2e2" width="1150" height="600" cellpadding="0" cellspacing="0">
    ...
    <tr valign="top">
        <td style="padding-left: 30px;">
            ...
            <?php
            $SQL = "SELECT `id`, `Name`, `Phone`, `Status` FROM `callback`";
            ...
            $inf .='<tr style="text-align: center; background-color: #9bd7d5; color: #fff; font-size: 20px;">
                    <td width="50px">id</td><td width="250px">Name</td><td>Phone</td><td>Status</td><td></td><td></td></tr>';
            ...
            for($i=0;$i<$count;$i++)
            {
                ...
                $inf .='<td width="250px">' .$str['Name'].'</td>';;
                $inf .='<td width="250px">' .$str['Phone'].'</td>';
                $inf .='<td width="250px">' .$str['Status'].'</td>';
                $inf .= '<td><form action="edit_callback.php" method="POST"><button type="submit" name="edit" value="'.$str['id'].'">Редактировать</button></form></td>
                <td><form action="delete_callback.php" method="POST"><button name="delete" type="submit" value="'.$str['id'].'">Удалить</button></form></td>';
            }
            $inf .='</tr></table>';
            echo $inf;
            ?>
